...
// Corrected divChildren function
function divChildren(path){
    let children = [];
    // The children are on the node, not the path
    path.node.children.forEach( (childNode) =>{
        // We only care about actual JSX elements, not text or expressions
        if (childNode.type === 'JSXElement') {
            const componentName = getComponentName(childNode.openingElement.name);
            
            // Now you can safely use the componentName
            const component = { // Use 'const component' instead of 'components'
                name: componentName,
                // You might want to extract attributes here as well if needed
                // attributes: extractAttributes(childNode.openingElement) 
            };
...
function extractComponentInfo(path,text){
    ...
    // Inside extractComponentInfo function
    if (name === "div"){
        // Pass the parent path directly
